...
// Recursive helper: builds a tree for entries starting at `start` up to `count`
// where all entries share the prefix at depth `depth` (number of '/' separators already consumed)
static int write_tree_recursive(IndexEntry *entries, int count, int depth, ObjectID *id_out) {
    ...
    while (i < count) {
        ...
        // Is there another slash? If yes, this is a subdirectory
        ...
        if (slash == NULL) {
            ...
            // It's a directory — get directory name
